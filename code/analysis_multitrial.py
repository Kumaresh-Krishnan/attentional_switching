...
def findBouts(raw_data, stimulus, pre, post):
    ...
    if valid < 0.9 or np.isnan(stim_rate):# or cut < 0.95:
        ...
        # valid_angles = valid_angles[np.abs(valid_angles) > thresh]

        if stimulus == 0:
            performance = (valid_angles < 0).sum() + 0.5*(valid_angles==0).sum()
            # performance = (valid_angles < -thresh).sum()
        else:
            performance = (valid_angles > 0).sum() + 0.5*(valid_angles==0).sum()
            # performance = (valid_angles > thresh).sum()
...
def main(experiment, pre, post, mutant):
    ...
    save_dir = path.Path() / '..' / experiment / f'data_bout_rate_no_fwd_pre_{pre}_post_{post}'
    hdf.savemat(save_dir, to_save, format='7.3', oned_as='column', store_python_metadata=True)

    return 0

def plotData(experiment, data, pre, post, model):
...
def plotStimRateCorrelation(experiment, data, pre, post, model):
...
def plotStimBaselineCorrelation(experiment, data, pre, post, model):
...
def quickDist(experiment, data, pre, post, model):
    ...
    print(np.nanmean(b_rates), np.nanstd(b_rates.ravel()), np.nanmean(br), np.nanstd(br.ravel()))
    f, ax = plt.subplots()
