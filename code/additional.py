...
import path
from scipy.stats import spearmanr, sem, binned_statistic, binom
...
def groupTFB(data, pre, post):
    ...
    f.savefig(data_path / '../results' / 'group_distribution.png')
    f.savefig(data_path / '../results' / 'group_distribution.pdf')

    plt.close(f)

    return

def explorePerf(data, pre, post):
    ...
    v, b = np.histogram(performance, range=(-1,1), bins=21)
    b = (b[1:] + b[:-1])*0.5
    ...
    f.savefig(data_path / '../results' / 'perf_distribution.pdf')
    f.savefig(data_path / '../results' / 'perf_distribution.png')

    plt.close(f)

    return

def individualPerf(data, pre, post):
    ...
    f.savefig(data_path / '../results' / 'engage_prop.pdf')
    f.savefig(data_path / '../results' / 'engage_prop.png')

    plt.close(f)

    return

def perfOrder(data, pre, post):
    ...
    sns.heatmap(performance, vmin=-1.0, vmax=1.0, cmap='coolwarm_r' ,center=0, mask=np.isnan(performance))
    ...
    f.savefig(data_path / '../results' / 'avg_trial_performance.pdf')
    f.savefig(data_path / '../results' / 'avg_trial_performance.png')

    plt.close(f)
    
    return

def rankedPerf(data, pre, post):
    ...
    f.savefig(data_path / '../results' / 'trial_performance_ranked.pdf')
    f.savefig(data_path / '../results' / 'trial_performance_ranked.png')

    plt.close(f)

    return

def rankedRate(data, pre, post):
    ...
    f.savefig(data_path / '../results' / 'trial_rate_ranked.pdf')
    f.savefig(data_path / '../results' / 'trial_rate_ranked.png')

    plt.close(f)

    return

def rateOrder(data, pre, post):
    ...
    f.savefig(data_path / '../results' / 'avg_trial_rate.pdf')
    f.savefig(data_path / '../results' / 'avg_trial_rate.png')

    plt.close(f)
    
    return

def rankedStim(data, pre, post):
    ...
    f.savefig(data_path / '../results' / 'trial_stim_ranked.pdf')
    f.savefig(data_path / '../results' / 'trial_stim_ranked.png')

    plt.close(f)

    return

def numBoutsOrder(data, pre, post):
    ...
    f.savefig(data_path / '../results' / 'dist_numbouts.pdf')
    f.savefig(data_path / '../results' / 'dist_numbouts.png')

    plt.close(f)
    
    return

def rankedNumBouts(data, pre, post):
    ...
    f.savefig(data_path / '../results' / 'trial_numbouts_ranked.pdf')
    f.savefig(data_path / '../results' / 'trial_numbouts_ranked.png')

    plt.close(f)

    return

def corrTFB(data_tfb, data_rate, pre, post):

    data_path = path.Path() / '..' / 'data'
    tmp = hdf.loadmat(data_path / data_tfb)
    ...
    f.savefig(data_path / '../results' / 'tfb_rate.pdf')
    f.savefig(data_path / '../results' / 'tfb_rate.png')

    plt.close(f)

    return

def rateNumBoutCorr(data, pre, post):
    ...
    f.savefig(data_path / '../results' / 'rate_numbouts_corr.pdf')
    f.savefig(data_path / '../results' / 'rate_numbouts_corr.png')

    plt.close(f)

    return

def perfNumBoutCorr(data, pre, post):
    ...
    f.savefig(data_path / '../results' / 'perf_numbouts_corr_binned.pdf')
    f.savefig(data_path / '../results' / 'perf_numbouts_corr_binned.png')

    plt.close(f)

    return

def stimNumBoutCorr(data, pre, post):
    ...
    f.savefig(data_path / '../results' / 'stim_numbouts_corr.pdf')
    f.savefig(data_path / '../results' / 'stim_numbouts_corr.png')

    plt.close(f)

    return

def shufflePerf(data, pre, post):
    ...
    f.savefig(data_path / '../results' / 'trial_performance_shuffled_ranked.pdf')
    f.savefig(data_path / '../results' / 'trial_performance_shuffled_ranked.png')

    plt.close(f)

    return

def cutCorr(data, pre, post):
    ...
    f.savefig(data_path / '../results' / 'cut_corr.pdf')
    f.savefig(data_path / '../results' / 'cut_corr.png')

    plt.close(f)

    return

def checkNan(data, pre, post):
...
def vecBoutPerf(data, pre, post):
    ...
    f.savefig(data_path / '../results' / 'vector_numbout_perf.pdf')
    f.savefig(data_path / '../results' / 'vector_numbout_perf.png')

    plt.close(f)

    return

def createData(data, pre, post):
...
def extremeNumBouts(data, pre, post):
    ...
    f.savefig(data_path / '../results' / f'extreme_bouts.pdf')
    f.savefig(data_path / '../results' / f'extreme_bouts.png')

    plt.close(f)

    return

def numBoutDist(data, pre, post):
    ...
    f.savefig(data_path/ '../results' / f'numbout_dist.pdf')
    f.savefig(data_path / '../results' / f'numbout_dist.png')

    plt.close(f)

    return

def binomialPerf(data, pre, post):
    ...
    f.savefig(data_path / '../results' / f'binomial_compare.pdf')
    f.savefig(data_path / '../results' / f'binomial_compare.png')
...
if __name__ == '__main__':
    ...
    data_tfb = f'data_tfb.mat'

    # groupTFB(data, pre, post) # Not for model
    # corrTFB(data, data_tfb, pre, post) # Not for model
    # explorePerf(data, pre, post)
    # perfOrder(data, pre, post)
    # individualPerf(data, pre, post)
    # rateOrder(data, pre, post)
    # rankedRate(data, pre, post)
    # rankedPerf(data, pre, post)
    # numBoutsOrder(data, pre, post)
    # rankedNumBouts(data, pre, post)
    # rateNumBoutCorr(data, pre, post)
    # perfNumBoutCorr(data, pre, post)
    # stimNumBoutCorr(data, pre, post)
    # rankedStim(data, pre, post)
    # shufflePerf(data, pre, post)
    # printVals(data, pre, post)
    # extremeNumBouts(data, pre, post) 
    # binomialPerf(data, pre, post)
    # numBoutDist(data, pre, post)
    # cutCorr(data, pre, post) # Not for model
    # checkNan(data, pre, post) # Not for model
    # checkCut(data, pre, post) # Not for model
    # vecBoutPerf(data, pre, post)
