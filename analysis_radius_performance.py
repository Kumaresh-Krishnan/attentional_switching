...
def plotData(experiment, pre, post):

    data_path = path.Path() / '..' / experiment
    tmp = hdf.loadmat(data_path / f'data_radius_pre_{pre}_post_{post}')
...
def radDist(experiment, pre, post):
...
if __name__ == '__main__':
    ...
    experiments = ['experimental_analysis'] # The baseline has results from full 150s
    for experiment in experiments:

        for pre, post in pairs:

            main(experiment, pre, post)
            plotData(experiment, pre, post)
            radDist(experiment, pre, post)
