...
def model(experiment):
    ...
    tmp = hdf.loadmat(data_path / 'data_bout_rate_pre_120_post_30.mat')
    ...
    hdf.savemat(data_path / f'data_params.mat', {'all_params':all_params, 'params':params, 'errs':errs, 'bins':b}, \
...
def filt(experiment):
    ...
    data_path = path.Path() / '..' / experiment
    tmp = hdf.loadmat(data_path / 'data_params.mat')
    params = tmp['params']
    b = tmp['bins']

    epart, gpart = parts(b, params[0], params[1])

    mod_score = gpart / (gpart + epart)
    ...
    f.savefig(data_path / 'parts_modeled_performance.pdf')
    f.savefig(data_path / 'parts_modeled_performance.png')
...
def bars(experiment):
    ...
    f.savefig(data_path / 'bar_param.pdf')
    f.savefig(data_path / 'bar_param.png')
